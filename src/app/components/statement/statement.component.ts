...
export class StatementComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void 
  {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (!this.key || !this.hash) {
        // redirect to home when params missing
  ...
  loadStatement() 
  {
    ...
    {
  // Invalid role -> redirect home instead of showing error
  this.router.navigate(['/']);
  return;      
    }
  ...
  // Any real backend error (wrong hash/company key etc.) -> redirect home
  this.router.navigate(['/']);
      }
    });
  }
