      ...
      <p-card styleClass="action-card">
        <div class="action-content">
          <div class="button-group">
            <p-button label="عرض كشف الحساب الكامل" icon="pi pi-file-text" size="large" styleClass="full-statement-btn" (onClick)="viewFullStatement()"></p-button>
            <p-button label="تحديث البيانات" icon="pi pi-refresh" size="large" styleClass="refresh-btn" (onClick)="refreshData()"></p-button>
          </div>
        </div>
      </p-card>

      <p-card *ngIf="statement.lastTransactionDate || statement.accountNumber || statement.status" styleClass="info-card">
...
export class ClientStatementComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.statementService.getClientTransactions(key, hash).subscribe({
  ...
  viewFullStatement(): void {
    // TODO: Implement navigation or dialog for full statement view
  }
